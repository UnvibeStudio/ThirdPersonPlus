...
public final class ThirdPersonEvents {
  public static void register() {
    ClientTickEvents.START_CLIENT_TICK.register(ThirdPersonEvents::onClientTickPre);
    ClientLifecycleEvents.CLIENT_STOPPING.register(ThirdPersonEvents::onClientStopping);
    // Fires when the local player entity is (re)loaded: initial join, respawn and dimension change.
    ClientEntityEvents.ENTITY_LOAD.register(ThirdPersonEvents::onClientEntityLoad);
    ...
    GameEvents.mouseScroll = ThirdPersonEvents::onMouseScrolled;
  }

  /**
   * Client tick 前
   *
   * @see ClientTickEvents#START_CLIENT_TICK
   */
  ...
  /**
   * Fires when the local player entity is (re)loaded on the client: initial join, respawn, and
   * dimension change. Replaces Architectury's separate join / respawn events.
   */
  private static void onClientEntityLoad(@NotNull Entity entity, @NotNull ClientLevel level) {
    ...
    if (config.is_mod_enabled) {
      resetPlayer();
      ThirdPerson.LOGGER.info("on local player load");
    }
    ...
    if (ThirdPerson.isAvailable()) {
      ...
      if (ThirdPersonStatus.isRenderingInThirdPerson()) {
        if (ThirdPerson.ENTITY_AGENT.isInteracting()) {
          ...
          ((MinecraftInvoker) Minecraft.getInstance()).invokePick(1f);
        }
      }
    }
  }
  ...
  /**
   * Adjust the camera distance with the mouse wheel while the distance-adjust key is held.
   *
   * @return {@code true} if the scroll was consumed (vanilla hotbar scrolling is then cancelled)
   */
  private static boolean onMouseScrolled(double amountY) {
